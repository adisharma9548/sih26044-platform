      ...
      <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 mb-4 p-4 bg-gray-50 rounded-lg">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                className="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
                rows={3}
                required
              />
            </div>
        ...
        <div className="space-y-3">
          {projects.length === 0 ? (
            ...
            projects.map((proj) => (
              <div key={proj._id} className="flex justify-between items-start p-3 border rounded-lg hover:bg-gray-50">
                <div>
                  <p className="font-medium">{proj.title}</p>
                  <p className="text-sm text-gray-600">{proj.description}</p>
                  {proj.link && (
                    <a href={proj.link} target="_blank" rel="noopener noreferrer" className="text-sm text-primary-600 hover:underline">
                      View Project
                    </a>
                  )}
                </div>
